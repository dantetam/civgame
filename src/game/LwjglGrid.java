...
public class LwjglGrid extends Grid {
	...
	public LwjglGrid(String playerCiv, double[][] terrain, int[][] biomes, int[][] resources, int[][] fields,
		...
		super(playerCiv, terrain, biomes, resources, fields, numCivs, numCityStates, difficultyLevel, numBarbarians,
				cutoff, seed);
	...
	public Vector3f get3DPositionFromTile(int r, int c) {
		float x = ((float) r + 0.5F) / (float) rows * Terrain.SIZE * 0.9F;
		float y = 0;
		float z = ((float) c + 0.5F) / (float) cols * Terrain.SIZE * 0.9F;
		return new Vector3f(x, y, z);
	}

	// The following five methods do not override old behavior. Rather, they extend
	// function to LWJGL systems
	// to ensure efficient updating of models in them.
	// The manager != null checks are to ensure that updating only happens after the
	// initial render (i.e. game starts)
