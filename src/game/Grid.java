...
public class Grid {
	...
	public Grid(String playerCiv, double[][] terrain, int[][] biomes, int[][] resources, int[][] fields, int numCivs,
			int numCityStates, int difficultyLevel, int numBarbarians, int cutoff, long seed) {
		...
		for (int r = 0; r < terrain.length; r++) {
			for (int c = 0; c < terrain[0].length; c++) {
				...
				if (terrain[r][c] >= cutoff) {
					// 2-4 fields
					tiles[r][c] = new Tile(this, "Land", (int) terrain[r][c], biomes[r][c], hill, resources[r][c],
							forest, r, c, (int) (Math.random() * 3) + 2);
					if (random > 0.98)
		...
		for (int i = 0; i < numCivs; i++) {
			...
			/*
			 * Civilization civ = new Civilization("Civilization " + Double.toString(
			 * Math.floor(Math.sqrt(System.currentTimeMillis()*rand.nextDouble())) ),new
			 * ArrayList<String>(),255,255,255);
			 */
			/*
			 * civ.r = (float)(rand.nextDouble()*255); civ.sR = civ.r; civ.g =
			 * (float)(rand.nextDouble()*255); civ.sG = civ.g; civ.b =
			 * (float)(rand.nextDouble()*255); civ.sB = civ.b;
			 */
			civ.sR = civ.r;
			civ.sG = civ.g;
			civ.sB = civ.b;
			...
			case 5:
				...
				break;
			default:
				System.out.println("Invalid difficulty: " + difficultyLevel);
				break;
			}
			...
			/*
			 * Tile t = findIsolated(); int r = t.row, c = t.col;
			 */
			...
			/*
			 * if (i == 0) //Give extra settlers to the player as a test for (int j = 0; j <
			 * 4; j++) { int nr,nc; while (true) { nr =
			 * (int)(rand.nextDouble()*tiles.length); nc =
			 * (int)(rand.nextDouble()*tiles[0].length); if (getTile(nr,nc).biome != -1)
			 * break; } addUnit(EntityData.get("Settler"),civs[i],nr,nc); }
			 */
		...
		for (int i = numCivs; i < numCivs + numCityStates; i++) {
			...
			civ.r = (float) (rand.nextDouble() * 255);
			civ.sR = 255;
			civ.g = (float) (rand.nextDouble() * 255);
			civ.sG = 255;
			civ.b = (float) (rand.nextDouble() * 255);
			civ.sB = 255;
			civ.revealed = new int[terrain.length][terrain[0].length];
			civ.opinions = new int[numCivs + numCityStates + numBarbarians];
			civs[i] = civ;
		...
		for (int i = numCivs + numCityStates; i < civs.length; i++) {
			...
			Civilization civ = new Civilization("Barbarians" + (i - numCivs - numCityStates + 1),
					new ArrayList<String>(), 0, 0, 0, 1, 0, 0);
			civ.r = 50;
			civ.sR = 0;
			civ.g = 50;
			civ.sG = 255;
			...
			/*
			 * int r,c; do { r = (int)(rand.nextDouble()*tiles.length); c =
			 * (int)(rand.nextDouble()*tiles[0].length); } while (tiles[r][c].biome == -1);
			 */

			/*
			 * for (int j = 0; j < 3; j++) { addUnit(EntityData.get("Settler"),civ,r,c); }
			 */
			civ.techTree.researched("Civilization").unlockForCiv(civ);
	...
	public boolean stackAttack(ArrayList<GameEntity> attacker, ArrayList<GameEntity> defender) {
		...
		for (int i = 0; i < attacker.size(); i++) {
			// attacker.get(i).attack(conflictSystem.attack(attacker.get(i),
			// defender.get(iDef)), defender.get(iDef), r, c);
		...
		// Some defenders are still alive, return that the attack has 'failed' i.e. no
		// passage
		return false;
	}
	...
	public ArrayList<Tile> adjacent(int r, int c) {
		...
		if (getTile(r + 1, c) != null) {
			temp.add(getTile(r + 1, c));
		}
		if (getTile(r - 1, c) != null) {
			temp.add(getTile(r - 1, c));
		}
		if (getTile(r, c - 1) != null) {
			temp.add(getTile(r, c - 1));
		}
		if (getTile(r, c + 1) != null) {
			temp.add(getTile(r, c + 1));
		}
		if (getTile(r + 1, c + 1) != null) {
			temp.add(getTile(r + 1, c + 1));
		}
		if (getTile(r + 1, c - 1) != null) {
			temp.add(getTile(r + 1, c - 1));
		}
	...
	public Tile bestToImprove(City c) {
		...
		for (Tile t : c.land) {
			...
			for (String impr : c.owner.techTree.allowedTileImprovements) {
				if (!EntityData.allowedTileImprovement(t, impr))
					continue;
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				...
				if (dist > maxDist || getTile(r, c).owner != null || getTile(r, c).biome == -1)
					...
					cityScores[r][c] = returnCityScoreNoOwner(r, c) * 5 - (int) (distBias * dist) * 2
							- (int) (distBias * capitalDist);
					/*
					 * out: for (int rr = r - 2; rr <= r + 2; rr++) { for (int cc = c - 2; cc <= c +
					 * 2; cc++) { Tile t = getTile(rr,cc); if (t == null) continue; if
					 * (t.improvement != null) { if (t.improvement instanceof City) {
					 * cityScores[r][c] = 0; break out; } else cityScores[r][c]--; } else { if
					 * (t.owner == null) cityScores[r][c] += 4; else cityScores[r][c] += 2; } } }
					 */
				}
			}
		}
