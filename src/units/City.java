...
public class City extends TileEntity {
	...
	public boolean raze;
	public int cityFocus; // Production focus: 0, general production; 1, science/trade; 2, military
							// production; 3, expansion
	...
	/*
	 * public City(TileEntity other) { super(other); population = 1; land = new
	 * ArrayList<Tile>(); workedLand = new ArrayList<Tile>(); //queue = null;
	 * queueTurns = 0; //owner = null; focus = "Growth"; }
	 */
	...
	public boolean built(String building) {
		if (building == null)
			System.err.println("null building");
		for (int i = 0; i < buildings.size(); i++) {
			// System.out.println(buildings.get(i));
			if (buildings.get(i).name.equals(building))
				return true;
		}
		return false;
	}
	...
	public void workTiles(int num) {
		...
		for (int i = 0; i < num; i++) {
			...
			// System.out.println(temp.get(index).row + " " + temp.get(index).col + " " +
			// this.location.row + " " + this.location.col);
			temp.remove(index);
			scores.remove(index);
		}
		// return returnThis;
	...
	public void findResources() {
		for (int i = 0; i < resources.length; i++)
			resources[i] = 0;
		// Record tiles with harvested resources as extra yield and record the number of
		// these special tiles
		for (int i = 0; i < workedLand.size(); i++) {
	...
	public static ArrayList<String> staticEvalReasons(Tile t, String improvement) {
		...
		/*
		 * for (int i = 0; i < t.fields.size(); i++) { Field field = t.fields.get(i); f
		 * = field.foodPercent != 0 ? (int)(f*field.foodPercent) : f; g =
		 * field.goldPercent != 0 ? (int)(g*field.goldPercent) : g; m =
		 * field.metalPercent != 0 ? (int)(m*field.metalPercent) : m; }
		 */
		return conditions;
	}
	...
	public double[] evaluate(Tile t, int cityFocus) {
		...
		if (cityFocus == -1) {
			...
			int score = 0;
			/*
			 * if (focus.equals("Growth")) { score = f*2 + g + m + r; } else if
			 * (focus.equals("Production")) { score = f + g + m*2 + r; } else if
			 * (focus.equals("Wealth")) { score = f + g*2 + m + r*2; } else if
			 * (focus.equals("Balanced")) { score = f + g + m + r; } else {
