...
public class MainGameLoop {
	...
	/*
	 * //counter clockwise vertices float[] vertices = { //Left bottom and top
	 * right, resp. -0.5f, 0.5f, 0f, -0.5f, -0.5f, 0f, 0.5f, -0.5f, 0f, 0.5f, 0.5f,
	 * 0f };
	 * 
	 * //order in which to transverse the vertices int[] indices = {0,1,3,3,1,2};
	 * 
	 * {0,0,0,1,1,1,1,0};
	 */
	...
	public MainGameLoop(CivGame game) {
		try {
			...
			texturePack = new WhiteTerrainTexturePack(backTexture, t1, t2, t3, t4, t5, t6, t7);
			 ...
			 * //order in which to transverse the vertices int[] indices = {0,1,3,3,1,2};
			 * 
			 * //respective u,v vertex of texture to map to float[] textureCoords =
			 * {0,0,0,1,1,1,1,0};
			 */
			...
			camera = new Camera();
			main.camera = camera; // manually assign this since the while loop belows halts all processes (i.e. no
									// stopping) in civgame

			// GuiTexture test = new GuiTexture(loader.loadTexture("partTexture"), new
			// Vector2f(0.5f,0.5f), new Vector2f(0.2f,0.2f));
			// main.guis.add(test);
			...
			main.menuSystem.forceUpdate(); // Set everything up correctly initially
			tick();
	...
	public void tick() {
		while (true) {
			...
			/*
			 * if (main.grid.civs.length > 1) for (int i = 0; i < 1; i++) {
			 * ArrayList<GameEntity> list = main.grid.civs[i].units; if (list.size() == 0)
			 * continue; GameEntity random = list.get(0); Tile t =
			 * main.menuSystem.getMouseHighlighted(); if (t != null) {
			 * main.grid.moveTo(random, t.row, t.col);
			 * System.out.println(random.location.row + " " + random.location.col); } }
			 */

			/*
			 * if (frameCount % 50 == 0) { if (main.grid.civs.length > 1) for (int i = 0; i
			 * < 1; i++) { ArrayList<GameEntity> list = main.grid.civs[i].units; if
			 * (list.size() == 0) continue; GameEntity random = list.get(0); Tile t =
			 * main.grid.findIsolated(); //Testing unit 'animations'
			...
			/*
			 * for (Entity en: levelManager.entities) { en.rotate(0,1F,0); }
			 */
	...
	public double[][] generateRoughTerrain(double[][] terrain, int multiply) {
				...
				if (t.biome == -1) {
					for (int nr = r * multiply; nr < r * multiply + multiply; nr++)
						for (int nc = c * multiply; nc < c * multiply + multiply; nc++)
							vertices[nr][nc] = 0;
					/*
					 * for (int nr = r*multiply; nr < r*multiply + multiply; nr++) for (int nc =
					 * c*multiply; nc < c*multiply + multiply; nc++) vertices[nr][nc] = (c +
					 * (float)(nc%multiply)/(float)multiply)*widthBlock;
					 */
				}
		...
		for (int r = 0; r < terrain.length; r++) {
			for (int c = 0; c < terrain[0].length; c++) {
				...
				/*
				 * if (t.shape == 2) { double[][] renderHill =
				 * map.generate(DiamondSquare.makeTable(5, 5, 5, 5, multiply), new double[]{0,
				 * 0, 2, 7, 0.7, 1}); renderHill = DiamondSquare.max(renderHill, 40); for (int
				 * nr = r*multiply; nr < r*multiply + multiply; nr++) for (int nc = c*multiply;
				 * nc < c*multiply + multiply; nc++) { float rough = (float)renderHill[nr -
				 * r*multiply][nc - c*multiply]; vertices[nr][nc] += rough*2D*Math.random() -
				 * rough; } } else if (t.shape == 1) { map = new DiamondSquare(temp1); long seed
				 * = (long)(System.currentTimeMillis()*Math.random()); map.seed(seed);
				 * double[][] renderHill = map.generate(DiamondSquare.makeTable(0, 0, 0, 0,
				 * multiply), new double[]{0, 0, 2, 4, 0.5, 1}); renderHill =
				 * DiamondSquare.max(renderHill, 13); for (int nr = r*multiply; nr < r*multiply
				 * + multiply; nr++) for (int nc = c*multiply; nc < c*multiply + multiply; nc++)
		...
		/*
		 * for (int i = 0; i < vertices.length; i++) { vertices[i][0].y = 0;
		 * vertices[0][i].y = 0; } for (int r = 0; r < vertices.length; r++) { for (int
		 * c = 0; c < vertices[0].length; c++) { System.out.print((int)vertices[r][c] +
		 * " "); } System.out.println(); } this.multiply = multiply; for (int nr = 0; nr
		 * < vertices.length; nr++) { for (int nc = 0; nc < vertices[0].length; nc++) {
		 * Tile t = main.grid.getTile(nr / multiply, nc / multiply); if (t != null &&
		 * t.biome == -1 && main.grid.adjacentLand(t.row, t.col).size() == 0) continue;
		 * //if (nr % multiply != 0 || nc % multiply != 0) continue; Point p =
		 * vertices[nr][nc]; if (p != null) { vertices[nr][nc] = new Point(p.x +
		 * Math.random()*4D - 2, p.y, p.z + Math.random()*4D - 2); } } }
		 */
		return vertices;
	}
